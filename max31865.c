...
// This is the hardware SPI function that handles all of the communications with the MAX31865s.
// The pin names are MOSI, MISO, and SCLK.  They are arbitrary, and can be almost any pins
// on the PSoC 5LP.  The chip select is done separately.
uint8_t MAX31865_spixfer(uint8_t x) {
  ...
  SPIM_WriteByte(x);
  CyDelayUs(50);
  while (0u == (SPIM_ReadTxStatus() & SPIM_STS_SPI_DONE) && i < 250)
  {
    i++;
    CyDelay(2);
  }
  reply = SPIM_ReadRxData();
